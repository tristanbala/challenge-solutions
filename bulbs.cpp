...
int Solution::bulbs(vector<int> &A) {
    ...
    int numSwitches = 0; // number of times we flipped a switch

    bool inverted = false; // tracks when elements to the right are inverted

    // check each element to see if it is 0 (or inverted)
    for(int i=0; i < A.size(); i++){

        if(A[i] == inverted){
            A[i] = !inverted;
            inverted = !inverted; // we pressed a switch so invert the rest
            numSwitches++;
        }
